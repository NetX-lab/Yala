...
/* Sample's Logic */
int flow_shared_counter(int nb_queues);
...
main(int argc, char **argv)
{
	...
	result = doca_argp_init("flow_shared_counter", NULL);
	...
	/* run sample */
	ret = flow_shared_counter(dpdk_config.port_config.nb_queues);
	if (ret < 0) {
		DOCA_LOG_ERR("flow_shared_counter sample encountered errors");
