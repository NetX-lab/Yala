...
#define NUMBER_OF_SAMPLE 100000
...
typedef struct comp_custom_dpdk {
	uint64_t rx_invalid;
	uint64_t rx_timeout;
	uint64_t rx_idle;
	uint64_t tx_busy;
	...
	uint64_t max_lat;
	uint64_t min_lat;
	uint64_t dst_buf_bytes; /* size of all dst bufs */
} comp_dpdk_stats_t;
...
void stats_print_update_simple(rb_stats_t *stats, int num_queues, double time, bool end);
