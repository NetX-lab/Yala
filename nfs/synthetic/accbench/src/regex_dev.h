...
acc_dev_register(rb_conf *run_conf)
{
	...
	funcs = rte_zmalloc(NULL, sizeof(acc_func_t), 0);
	if (!funcs) {
	...
	*/
	case COMP_DEV_DPDK_COMP:
		ret = comp_dev_dpdk_comp_reg(funcs);
		if (ret)
			return ret;
		break;
...
static inline void
dev_force_batch_push(rb_conf *run_conf, uint16_t rx_port, int qid, dpdk_egress_t *dpdk_tx, regex_stats_t *stats)
{
	acc_func_t *funcs = run_conf->regex_dev_funcs;
...
static inline void
dev_force_batch_pull(rb_conf *run_conf, int qid, dpdk_egress_t *dpdk_tx, regex_stats_t *stats)
{
	acc_func_t *funcs = run_conf->regex_dev_funcs;
